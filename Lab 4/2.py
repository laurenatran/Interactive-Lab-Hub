...
while True:
   ...
   # if not buttonA.value and not buttonB.value:
    ...
    if buttonA.value and buttonB.value: 
        image = Image.new("RGB", (width, height))
        draw = ImageDraw.Draw(image)
        draw.rectangle((0, 0, width, height), outline=0, fill=(230, 93, 16))
        #draw.text((x, y), "Button A Pressed", font=font, fill="#FFFFFF")
        #draw.text((x+5, y), "Add milk", font = font, fill = 15)
        draw.text((x+5, y+20), "Coffee", font = font, fill = 15)
        draw.text((x+5, y+40), "is ready!", font = font, fill = 15)
        display.image(image, rotation)
        display.image(image, rotation)
    # Button A Pressed
    if buttonB.value and not buttonA.value:  # just button A pressed
        ...
        draw.text((x+5, y+20), "Coffee is ready!", font = font, fill = 15)
